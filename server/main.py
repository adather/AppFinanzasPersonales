...
# Pure statistical analysis (category stats, robust anomaly detection,
# behavioral patterns, timeline, spending tiers) — no Gemini call, so this
# works even without an API key configured.
@app.post("/api/analytics/summary")
async def analytics_summary(body: AnalyticsSummaryRequest) -> dict[str, Any]:
    try:
        data = build_analytics_summary(
            body.currentTransactions,
            body.previousTransactions,
            body.anomalyThreshold,
        )
        return {"success": True, "data": data}
    except Exception as error:  # noqa: BLE001
        print(f"Error computing analytics summary: {error}")
        return JSONResponse(
            status_code=500,
            content={"success": False, "error": "Error al calcular estadísticas", "message": str(error)},
        )
...
async def parse_receipt(body: ParseReceiptRequest) -> dict[str, Any]:
    ...
    async def generate(model: str):
        # Resolved lazily (not before the try/except in run_json_endpoint)
        # so a missing GEMINI_API_KEY surfaces as the same friendly error
        # envelope as any other Gemini failure, not a raw 500.
        client = get_client()
    ...
    return await run_json_endpoint(generate, model_to_use, "Error processing receipt", "Error al procesar el recibo con IA")
...
async def parse_verbal(body: ParseVerbalRequest) -> dict[str, Any]:
    ...
    return await run_json_endpoint(generate, model_to_use, "Error parsing verbal expense", "Error al interpretar el gasto verbal")
...
async def analyze_finances(body: AnalyzeFinancesRequest) -> dict[str, Any]:
    ...
    return await run_json_endpoint(generate, model_to_use, "Error analyzing finances", "Error al generar análisis con IA")
...
async def chat_advisor(body: ChatAdvisorRequest) -> dict[str, Any]:
    ...
    # Not run_json_endpoint: this endpoint replies with free-form text, not
    # a parsed JSON object, so it keeps its own envelope.
    try:
        ...
        return ai_error_response(error, "Error en el chat de asesoría")
...
async def savings_goal_advice(body: SavingsGoalAdviceRequest) -> dict[str, Any]:
    ...
    return await run_json_endpoint(
        generate, model_to_use, "Error in savings goal advice", "Error al generar consejos para la meta de ahorro"
    )
